...
class MyTestCase(unittest.TestCase):
    def test_given_case(self):
        input_table = pd.DataFrame({'product_id': [0, 1, 2, 3, 4], 'low_fats': ['Y', 'Y', 'N', 'Y', 'N'],
                                    'recyclable': ['N', 'Y', 'Y', 'Y', 'N']})
        result = find_products(input_table)
        expected = pd.DataFrame({'product_id': [1, 3]})
        self.assertEqual(is_equal_dataframes(result, expected), True)

    def test_different_data(self):
        input_table = pd.DataFrame({'product_id': [0, 1, 2, 3, 4], 'low_fats': ['Y', 'N', 'N', 'Y', 'Y'],
    ...
    def test_single_entry_true(self):
        input_table = pd.DataFrame({'product_id': [0], 'low_fats': ['Y'],
                                    'recyclable': ['Y']})
        result = find_products(input_table)
    ...
    def test_single_entry_false_1(self):
        input_table = pd.DataFrame({'product_id': [0], 'low_fats': ['N'],
                                    'recyclable': ['N']})
        result = find_products(input_table)
